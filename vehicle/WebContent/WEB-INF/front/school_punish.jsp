											...
											<tbody>
												<c:forEach items="${schoolList}" var="school">
													<tr>
														...
														<td><c:if test="${school.SCHOOL_STATE==2}">正在运营/允许报名</c:if>
															<c:if test="${school.SCHOOL_STATE==3}">正在运营/禁止报名</c:if> <c:if
																test="${school.SCHOOL_STATE==4}">停止运营/允许报名</c:if>
																<c:if test="${school.SCHOOL_STATE==5}">停止运营/禁止报名</c:if></td>
														<td><c:if test="${school.SCHOOL_STATE==2}">
																<button class="btn btn-sm btn-danger m-b-5 m-t-5" onclick='punish(${school.SCHOOL_ID},"4")'>停止运营</button>&nbsp;
															<button class="btn btn-sm btn-danger m-b-5 m-t-5" onclick='punish(${school.SCHOOL_ID},"3")'>禁止报名</button>
															</c:if>
															<c:if test="${school.SCHOOL_STATE==3}">
															<button class="btn btn-sm btn-danger m-b-5 m-t-5" onclick='punish(${school.SCHOOL_ID},"5")'>停止运营</button>&nbsp;
															<button class="btn btn-sm btn-primary m-b-5 m-t-5" onclick='punish(${school.SCHOOL_ID},"2")'>恢复报名</button>
															</c:if>
															<c:if test="${school.SCHOOL_STATE==4}">
															<button class="btn btn-sm btn-primary m-b-5 m-t-5" onclick='punish(${school.SCHOOL_ID},"2")'>恢复运营</button>&nbsp;
															<button class="btn btn-sm btn-danger m-b-5 m-t-5" onclick='punish(${school.SCHOOL_ID},"5")'>禁止报名</button>
															</c:if>
															<c:if test="${school.SCHOOL_STATE==5}">
															<button class="btn btn-sm btn-primary m-b-5 m-t-5" onclick='punish(${school.SCHOOL_ID},"3")'>恢复运营</button>&nbsp;
															<button class="btn btn-sm btn-primary m-b-5 m-t-5" onclick='punish(${school.SCHOOL_ID},"4")'>恢复报名</button>
															</c:if>
															</td>
													</tr>
												</c:forEach>
			...
			$.ajax({
				...
				success:function(data){
					if(data.status=="y"){
						...
						window.location.href="${pageContext.request.contextPath}/front/control/schoolPunishList.handler";
					}else{
						window.alert(data.info);
					}
				}
